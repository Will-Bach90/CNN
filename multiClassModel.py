...
# # Define paths
train_path = 'pneumonia-dataset-annotated/train'
test_path = 'pneumonia-dataset-annotated/test'
valid_path = 'pneumonia-dataset-annotated/val'
...
def create_data_generators(batch_size, img_height, img_width):
    ...
    train_generator = train_datagen.flow_from_directory(
        ...
        class_mode='categorical',  # Categorical labels (three classes)
...
def create_cnn_model(img_height, img_width):
    ...
    # Add a Dense (fully connected) layer with a softmax activation function for multi-class classification output.
    model.add(Dense(activation='softmax', units=3))

    # Compile the model: 'adam' optimizer and 'categorical_crossentropy' loss.
...
multi_model = create_cnn_model(img_height, img_width)
...
# multi_model = load_model('multi_model.h5')

# Evaluate the model on the test data and print the testing accuracy
test_accu = multi_model.evaluate(test_gen)
print('Testing accuracy:', test_accu[1] * 100, '%')

# Generate predictions for the test data
predictions = multi_model.predict(test_gen, verbose=1)
...
cmDF = pd.DataFrame(cm,
                     index=["Actual Normal", "Actual Viral", "Actual Bacterial"],
                     columns=["Predicted Normal", "Predicted Viral", "Predicted Bacterial"])

# Display the confusion matrix
plt.figure(figsize=(8, 6))  
sns.heatmap(cmDF, annot=True, cmap="crest", fmt="g")
...
print(classification_report(test_gen.classes, predicted_classes, target_names=["Normal", "Viral", "Bacterial"]))